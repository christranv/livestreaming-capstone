...
namespace Stream.API.Application.Behaviors
{
    public class TransactionBehaviour<TRequest, TResponse> : IPipelineBehavior<TRequest, TResponse>
    {
        private readonly ILogger<TransactionBehaviour<TRequest, TResponse>> _logger;
        private readonly StreamContext _dbContext;
        private readonly IStreamingIntegrationEventService _streamingIntegrationEventService;

        public TransactionBehaviour(StreamContext dbContext,
            IStreamingIntegrationEventService streamingIntegrationEventService,
            ILogger<TransactionBehaviour<TRequest, TResponse>> logger)
        {
            _dbContext = dbContext ?? throw new ArgumentException(nameof(StreamContext));
            _streamingIntegrationEventService = streamingIntegrationEventService ??
                                                throw new ArgumentException(nameof(streamingIntegrationEventService));
